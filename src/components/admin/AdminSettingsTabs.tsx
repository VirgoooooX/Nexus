...
import { Settings, Rss, TerminalSquare, ScrollText } from 'lucide-react';
...
export function AdminSettingsTabs({ settingsProps, readflowServerUrl }: AdminSettingsTabsProps) {
    const [activeTab, setActiveTab] = useState<'settings' | 'prompt' | 'readflow' | 'logs'>('settings');
        ...
        <div className="space-y-8">
            ...
            <nav className="flex flex-wrap gap-1 bg-stone-100 p-1 rounded-lg w-fit">
                ...
                <button type="button" onClick={() => setActiveTab('logs')}
                    className={`inline-flex items-center gap-2 px-5 py-2 text-sm font-bold rounded-md transition-all ${activeTab === 'logs' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>
                    <ScrollText className="w-3.5 h-3.5" /> 系统日志
                </button>
            </nav>
